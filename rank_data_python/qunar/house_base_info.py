...
房屋基础信息处理
"""
data_dir="/home/q/tujia/rank_data_v2.0/data/base"
...
class BaseInfo(PushData):
    ...
    def region_price_level(self):
        ...
        for line in open(setting['region_price_path'], 'r', encoding='utf-8').readlines():
            try:
                houseid, area_price_level, person_price_level = line.strip().split('\t')[:3]
        ...
        line_len = len(self.region_price_level_dict.keys())
        if line_len < 1000000:
            raise Exception('区域价格水平量级异常: %s' % (line_len))
    ...
    def comment_tag(self):
        ...
        for line in open(setting['comment_tag_path'], 'r', encoding='utf-8').readlines():
            try:
                houseid, taginfo = line.strip().split('\t')[:2]
                taginfo = eval(taginfo)
                # 对其保留2位小数
                if taginfo is not None:
                    for k,v in taginfo.items():
                        taginfo[k] = round(float(v), 2)
